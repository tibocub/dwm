...
diskfree() {
  freeHDD=$(df -Ph | grep "/dev/sda5" | awk {'print $4'})
  freeSSD=$(df -Ph | grep "/dev/sdb2" | awk {'print $4'})

  echo "Free Space :   $freeSSD   $freeHDD"
}

main() {
	while true; do
    xsetroot -name " $(diskfree) | $(ram) | $(cpu) | $(network) | $(volume_alsa) | $(clock)"
		sleep 1
	done
}
